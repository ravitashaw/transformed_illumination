...
class Illumination(nn.Module):
    ...
    def forward(self, data_cubes, channel_weights):
        ...
        images = data_cubes.view(-1, self.num_leds, self.image_shape[-2], self.image_shape[-1])
...
class FixedIllumination(nn.Module):
    ...
    def forward(self, data_cubes, iter):
        # data cubes are of shape: B x C x (H x W)
        # led_weights are of shape: B x num_leds
        # channel weights are: B x C x 1
        images = data_cubes.view(-1, self.num_leds, self.image_shape[-2], self.image_shape[-1])
        if iter == 0:
            ...
            led_weights = self.ill_1[0].weight.view(1, 1, self.num_leds).expand(len(data_cubes), 1,
                                                                                               self.num_leds)
            output = self.ill_1(images)
        else:
            led_weights = self.ill_2[0].weight.view(1, 1, self.num_leds).expand(len(data_cubes), 1,
                                                                                               self.num_leds)
